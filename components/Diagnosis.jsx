...
export default class Diagnosis extends Component {
  ...
  navigate = (obj) => {
    this.props.navigation.navigate("detailedDiagnosis", { data: obj });
  };
  ...
  render() {
    ...
    return (
      <ScrollView style={{ padding: 15 }}>
        ...
        {data.predicted_diagnoses.map((obj, index) => (
          <View
            key={`diagnosis-${index}`}
            style={{
              marginTop: 20,
          ...
          >
            <Text style={{ fontSize: 26, fontWeight: "bold" }}>{obj?.common_name}</Text>
            <Text style={{ fontSize: 15, fontWeight: "bold", color: "grey" }}>{obj?.hosts}</Text>
            ...
            {obj?.symptoms_short.map((symptom, symptomIndex) => (
              <View
                key={`symptom-${index}-${symptomIndex}`}
            ...
            <View style={styles.buttonContainer}>
              <TouchableOpacity style={styles.button} onPress={() => this.navigate(obj)}>
